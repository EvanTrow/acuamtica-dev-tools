...
export default function Instances() {
  ...
  const [instanceLocation, setInstanceLocation] = React.useState(
    window.appSettings.instanceLocation
  );
  const [instanceLocationValid, setInstanceLocationValid] =
    React.useState(true);

  const [buildLocation, setBuildLocation] = React.useState(
    window.appSettings.buildLocation
  );
  const [buildLocationValid, setBuildLocationValid] = React.useState(true);
  const [extractMsi, setExtractMsi] = React.useState(
    window.appSettings.extractMsi
  );
  const [lessmsiPath, setLessmsiPath] = React.useState(
    window.appSettings.lessmsiPath
  );
  ...
  const updateInstanceLocation = async (instanceLocation: string) => {
    ...
    if (validPath || instanceLocation === '') {
      ...
      window.electronAPI.execSQL(
        `UPDATE settings SET instanceLocation = "${instanceLocation}";`
      );
    }
  };
  ...
  const updateBuildLocation = async (buildLocation: string) => {
    ...
    if (validPath || buildLocation === '') {
      ...
      window.electronAPI.execSQL(
        `UPDATE settings SET buildLocation = "${buildLocation}";`
      );
    }
  };
  ...
  const updateExtractMsi = async (extractMsi: boolean) => {
    ...
    window.electronAPI.execSQL(
      `UPDATE settings SET extractMsi = ${extractMsi ? 1 : 0};`
    );
  };
  ...
  const updateLessmsiPath = async (lessmsiPath: string) => {
    ...
    if (validPath && lessmsiPath.endsWith('.exe')) {
      ...
      window.electronAPI.execSQL(
        `UPDATE settings SET lessmsiPath = "${lessmsiPath}";`
      );
    }
  };
          ...
          <Card sx={{ minWidth: 275 }}>
            <CardContent>
              ...
              <TextField
                ...
                variant="outlined"
                helperText={
                  instanceLocationValid
                    ? 'Folder path where your Acuamtica instances are installed.'
                    : 'Invalid Path'
                }
          ...
          <Card sx={{ minWidth: 275 }}>
            <CardContent>
              ...
              <TextField
                ...
                helperText={
                  buildLocationValid
                    ? 'Folder path where your Acuamtica builds are stored.'
                    : 'Invalid Path'
                }
                value={buildLocation}
              ...
              <FormGroup>
                <FormControlLabel
                  control={
                    <Switch
                      checked={extractMsi}
                      onChange={(e) => updateExtractMsi(e.target.checked)}
                    />
                  }
                  label="Extract MSI"
                />
              ...
              <FormHelperText>
                Extract contents of MSI folder when download completes
              </FormHelperText>
              <br />
              {extractMsi ? (
                <TextField
                  ...
                  variant="outlined"
                  helperText={
                    lessmsiPathValid ? 'Path to lessmsi.exe' : 'Invalid Path'
                  }
                  value={lessmsiPath}
