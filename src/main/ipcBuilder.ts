...
export default class IpcBuilder {
	...
	buildIpc() {
		...
		ipcMain.handle('getSettings', async (event) => {
			return new Promise((resolve, reject) => {
				this.db.all('SELECT * FROM settings', [], (err, rows) => {
					if (err) {
						SendToast(this.mainWindow, 'IPC Error! getSettings > ' + err.message, 'error');
			...
			try {
				this.db.all(query, [], (err, rows) => {
					if (err) {
						...
						SendToast(this.mainWindow, 'IPC Error! sql > ' + err.message, 'error');
					}
				});
			} catch (e) {
		...
		ipcMain.handle('getInstances', async (event) => {
			return new Promise((resolve, reject) => {
				this.db.all('SELECT * FROM instances', [], (err, rows) => {
					if (err) {
						SendToast(this.mainWindow, 'IPC Error! getInstances > ' + err.message, 'error');
						reject(err.message.toString());
					} else {
						resolve(rows);
					}
		...
		ipcMain.handle('getBuilds', async (event) => {
			return new Promise((resolve, reject) => {
				try {
					this.db.all('SELECT buildLocation FROM settings', [], async (err, rows) => {
						if (err) {
							SendToast(this.mainWindow, 'IPC Error! getBuilds > ' + err.message, 'error');
							reject(err.message.toString());
						} else {
							let folders: string[] = await fs.promises.readdir(rows[0].buildLocation);
