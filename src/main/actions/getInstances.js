...
export default async function GetInstances(mainWindow, database) {
  try {
    ...
    sitesData =
      sitesData.configuration['system.applicationHost'].sites.site.application;
    ...
    await asyncForEach(sitesData, async (site, i) => {
      ...
      if (
        ...
        acumaticaVersion = acuConfig.configuration.appSettings.add.find(
          (a) => a._attributes.key == 'Version'
        )._attributes.value;

        try {
          var connectionString =
            acuConfig.configuration.connectionStrings.add.find(
              (c) => c._attributes.name == 'ProjectX'
            )._attributes.connectionString;
